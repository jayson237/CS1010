...
do
	...
	then
		make_temp
		./"$PROG" < "inputs/$PROG.$i.in" 2> /dev/null | head -c 50MB > "$out"
		status="$?"
		if [ "$status" -ne "0" ]
		then
...
then
	if [ "$PROG" == "id" ] || [ "$PROG" == "days" ]; then
		check_has_branch $PROG
		if [ "$g_has_branch" -eq 1 ]; then
			echo "$PROG: failed.  Conditional statement detected."
		else
			echo "$PROG: passed"
		fi
	elif [ "$PROG" == "max" ]; then
		check_has_loop $PROG "find_max"
		if [ "$g_has_loop" -eq 1 ]; then
			echo "$PROG: failed.  Loop detected."
		else
			echo "$PROG: passed"
		fi
	else
