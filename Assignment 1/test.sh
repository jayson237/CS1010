...
function check_has_loop {
  local problem=$1
  got_while=`clang-check --ast-dump $problem.c 2> /dev/null| grep WhileStmt |   wc -l`
  got_for=`clang-check --ast-dump $problem.c 2> /dev/null| grep ForStmt | wc -l`
  got_do_while=`clang-check --ast-dump $problem.c 2> /dev/null| grep DoStmt |   wc -l`
...
then
	...
	then
		check_has_function digits "sum_of_digits_cubed" "long (long)"
		if [ "$g_has_function" -eq 0 ]
		then
			echo "$PROG: failed. The expected function sum_of_digits_cubed cannot be found"
		else
			check_has_loop digits
			if [ "$g_has_loop" -eq 1 ]
			then
				echo "$PROG: failed. Found loops in the solution."
			else
				echo "$PROG: passed"
			fi
		fi
	...
	then
		check_has_function suffix print_with_suffix "void (long)"
		if [ "$g_has_function" -eq 0 ]
		then
			echo "$PROG: failed. The expected function print_with_suffix cannot be found"
		else
			echo "$PROG: passed"
		fi
	else
